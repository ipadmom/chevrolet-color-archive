...
| `generations.parquet` | One contiguous display, platform band, or exact program timeline per model | 1,007 |
| `model_years.parquet` | One catalogued model and model-year pair | 1,792 |
| `model_year_generation_memberships.parquet` | One generation, specialty overlay, or exact program partition attached to a model-year | 2,337 |
| `platform_eras.parquet` | One sourced base, platform, or era band | 218 |
| `color_identities.parquet` | One normalized color timeline identity within a model generation | 1,832 |
| `color_availability.parquet` | One source-backed model, year, and color listing | 2,146 |
...
| `sources.parquet` | One canonical source URL | 2,883 |
| `source_revisions.parquet` | One immutable file revision of a source | 1,869 |
| `evidence_claims.parquet` | One exact source-revision and locator claim for a published availability row | 2,146 |
| `source_links.parquet` | One source-to-claim citation | 28,913 |
| `photo_assets.parquet` | One archived Wikimedia Commons original | 303 |
| `model_photo_links.parquet` | One model or exact-year photo association | 305 |
...
- `model_year_research.parquet` distinguishes 85 complete `color_chart_verified`
  model-years, six `reviewed_qualified_historical_table` years, 69 qualified
  palette-union years, 42 years whose strongest status is
  `reviewed_specialty_palette_subset`, four reviewed no-chart years, and 1,586
...
- Rows with `claim_status = published_qualified_palette_union` preserve the
  ...
  supplies. This build has 568 qualified-palette rows across 69 model-years.
- Rows with `claim_status = published_specialty_palette_subset` preserve 601
  exact, visually reviewed specialty or public-safety listings across 66
  application model-years. In 42 of those model-years, the specialty subset is
  the strongest research status. The other 24 coexist with a separately complete or
...
- `model_year_generation_memberships.parquet` makes every generation assignment
  explicit. Each model-year has exactly one primary membership. There are 541
...
`application_type` is non-null for all 2,146 availability rows. It describes
...
|---|---:|---|
| `manufacturer_listed` | 1,541 | Ordinary manufacturer-listed color availability, including regular charts and qualified palette unions. All rows of this type are outside the specialty subset. |
...
| `special_equipment_option_paint` | 262 | An SEO paint row for an exact police, special-service, or retail-and-fleet program. Read `availability_state` for open, planned, unavailable, or closed timing. |
...
| **All availability rows** | **2,146** | Every source-backed color availability row in the current export. |

Of the 2,146 rows, 601 have
